...
export const Lecturers: React.FC<LecturersProps> = ({ lecturerList: propLecturerList }) => {
  ...
  const filteredLecturers = lecturersList.filter((lecturer) => {
    const matchesProdi = selectedProdi === 'ALL' || lecturer.studyProgram === selectedProdi;
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 sm:p-6 border border-slate-200 dark:border-slate-700 shadow-sm mb-10 flex flex-col lg:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full lg:w-80">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2.5 text-xs rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-red-500 transition-all"
            />
            {searchQuery && (
              <button 
          ...
          {/* Prodi & Expertise Filter Controls */}
          <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
            {/* Filter Prodi Dropdown */}
            <select
              value={selectedProdi}
              onChange={(e) => setSelectedProdi(e.target.value)}
              className="px-3 py-2 text-xs font-bold rounded-xl border bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-red-500 cursor-pointer"
            >
            ...
            <div className="flex flex-wrap items-center gap-1">
              {[
                { code: 'ALL', label: 'Semua Bidang' },
                { code: 'AI', label: 'AI & Data' },
                { code: 'CyberSecurity', label: 'CyberSec' },
                { code: 'RPL', label: 'Software' },
                { code: 'IoT', label: 'IoT & Elektro' }
              ].map((item) => (
                <button
                  key={item.code}
                  onClick={() => setSelectedExpertise(item.code)}
                  className={`px-3 py-1.5 text-xs font-bold rounded-xl border transition-all cursor-pointer ${
                    selectedExpertise === item.code
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredLecturers.map((lecturer) => (
            <div
              ...
              className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md hover:border-red-500/60 transition-all cursor-pointer flex flex-col justify-between group"
            >
              <div>
                <div className="flex items-center gap-4 mb-4">
                  <img
                    ...
                    className="w-16 h-16 rounded-2xl object-cover border-2 border-slate-100 dark:border-slate-700 group-hover:scale-105 transition-transform shrink-0"
                  />
                  <div className="min-w-0 flex-1">
                    ...
                    <h3 className="font-bold text-slate-900 dark:text-white text-sm sm:text-base mt-1 line-clamp-1 group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors">
                      {lecturer.name}
                    </h3>
                ...
                <div className="space-y-2 text-xs border-t border-slate-100 dark:border-slate-700/80 pt-3">
                  <div className="flex items-center gap-2 text-slate-600 dark:text-slate-300">
                    ...
                    <span className="truncate">{lecturer.lab}</span>
                  </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    NIDN: {activeLecturerModal.nidn} • {activeLecturerModal.lab}
                  </p>
                </div>
              </div>
              <button
